...
```{r przygotowanie_danych, warning=F, message=F, echo = F}
library(neuralnet)
...
### Sieć trzywarstwowa
```{r siec 3,3,3, warning=F, message=F}
bledy = 0
...
```{r siec 3-2, warning=F, message=F}
bledy = 0
...
```{r kroswalidacja LDA 10, warning = F, message=F}
library(MASS)
...
```{r kroswalidacja LDA N, warning = F}
...
```{r kroswalidacja regresja, warning = F, message=F}
# 1. KROSWALIDACJA 10-KROTNA
bledy_10 = 0
...
```{r kroswalidacja MLP 10, warning = F, message=F}
# 1. KROSWALIDACJA 10-KROTNA
bledy_10 = 0
